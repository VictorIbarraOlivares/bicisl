	...
	<table class="table display" width="100%" cellpadding="0" cellspacing="0" id="datatable_bicicletas">
		...
		<tbody>
			@foreach($bikes as $bike)
				<tr>
					<td>
						<a href="{{ route('admin.users.detalle', $bike->usuario ) }}" style="color: black;"  title="{{ $bike->dueño }}">{{ $bike->dueño }}</a>
					</td>
					<td>
						@if($bike->activa == 1)
					...
					<td>
						...
						<a href="{{ route('admin.bicicletas.detalle', $bike->id) }}" class="btn btn-info" title="Detalles"><span class="glyphicon glyphicon-eye-open" aria-hidden="true" title="Detalles" style="color:black;"></span></a>
						...
						@if($bike->activa == 0)
							<a title="Eliminar" data-role="{{ $bike->id }}" class="btn btn-danger eliminar-data" data-target="#miModalEliminar" ><i class="fa fa-trash" aria-hidden="true" title="Eliminar" style="color:black;"></i></a>
